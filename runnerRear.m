function [FOPT, planet_d1_act, planet_d2_act, sun_d1_act, ring_d2_act, ...
    dp, gr_act] = runnerRear(pop_factor, GR, planets_diameter_axis)
...
%% Executables
vlb = [planet_extrusion_d, planet_extrusion_d, 1];
vub = [0.45 * bearing_d, 0.45 * bearing_d, 4];
bits = [16, 16, 2];
l = sum(bits);
...
[X,FOPT,STATS,NFIT,FGEN,LGEN,LFIT] = GA550('theFitnessRear', ...
    [], options, vlb, vub, bits, bearing_d, GR, planets_diameter_axis, ...
    sun_extrusion_d);
...
planet_d1 = X(1);
planet_d2 = X(2);
sun_d1 = planets_diameter_axis - planet_d1;
dp = 14 + 2 * X(3);
